...
import { Activity, AlertTriangle, ArrowUpRight, RadioTower } from "lucide-react";
...
export default async function DevNav() {
	...
	if (apiTarget) {
		try {
			...
			healthError = err instanceof Error ? err.message : "Health check unavailable";
		}
	} else {
		...
		<header className="relative z-50 border-b border-white/10 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 text-slate-100 shadow-lg">
			<div className="mx-auto flex w-full max-w-6xl flex-wrap items-center gap-4 px-6 py-4">
				...
				<nav className="flex flex-1 flex-wrap items-center justify-start gap-2 text-sm sm:justify-center">
					{navLinks.map((link) => (
						<Button key={link.href} variant="ghost" size="sm" asChild className="text-slate-100 hover:bg-white/10">
					...
					<Button
						...
						asChild
					>
						<a href="https://github.com/danmuck/edgectl" target="_blank" rel="noreferrer">
							Docs
				...
				<div className="flex flex-col items-start gap-1 text-xs sm:items-end">
					<div className="flex items-center gap-2">
						...
						{versionLabel && <span className="font-mono text-[11px] text-slate-300">{versionLabel}</span>}
						<ThemeToggleButton />
					</div>
					<span className="text-[11px] uppercase tracking-wide text-slate-400">{uptimeLabel}</span>
				</div>
			</div>
